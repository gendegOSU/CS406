...
<?php
    ...
    if (isset($_POST["updateJobId"])) {
        ...
        $update_job_query = $cford_db->prepare("UPDATE Jobs SET status = ?, customerId = ?, startDate = ?, endDate = ?, locationName = ?, streetAddress = ?, city = ?, state = ?, comments = ?  WHERE jobId = ?;");
    ...
    if (isset($_POST["updateRoleTaskRoleId"])) {
        ...
        $check_task_role_query = $cford_db -> prepare("SELECT * FROM TasksRoles WHERE roleId = ? and taskId = ?;");
        $check_task_role_query = $cford_db -> bind_param("ii",$_POST['roleId'], $_POST['updateRoleTaskId']);
        $check_task_role_query = $cford_db ->execute();
        $check_task_role = $check_task_role_query -> get_result().fetch_assoc();
    }

        if (isset($check_task_role["taskId"], $check_task_role["roleId"])) {

        $update_tasksRoles_query = $cford_db->prepare("UPDATE TasksRoles SET roleId = ?, roleQuantity = ?  WHERE taskRoleId = ?;");
    ...
    if (isset($_POST['updateEmployeeTaskRoleId'])) {
        $update_TasksRolesEmployees_query = $cford_db->prepare("UPDATE TasksRolesEmployees SET employeeId = ? WHERE taskRoleId = ?;");
...
if (isset($_POST['UpdateEquip'])) {
    ...
    if (isset($check_task_equip["taskId"], $check_task_equip["equipmentId"])) {
        $update_equip_query = $cford_db->prepare("UPDATE TasksEquipment SET quantityUsed = ? WHERE taskId = ? AND equipmentId = ?;");
    ...
    if(isset($_POST['updateType'])) {
        ...
        if($_POST['updateType'] == "TaskTypes") {
            $update_query = $cford_db->prepare("UPDATE TaskTypes SET taskTypeName = ? WHERE taskTypeId = ?;");
        ...
        if($_POST['updateType'] == "Customers") {
            $update_query = $cford_db->prepare("UPDATE Customers SET customerName = ?, phoneNumber = ?, email = ? WHERE customerId = ?;");
        ...
        if($_POST['updateType'] == "Employees") {
            $update_query = $cford_db->prepare("UPDATE Employees SET firstName = ?, lastName = ? WHERE employeeId = ?;");
        ...
        if($_POST['updateType'] == "EmployeesRoles") {
            $update_query = $cford_db->prepare("UPDATE EmployeesRoles SET roleId = ? WHERE employeeId = ? AND roleId = ?;");
        ...
        if($_POST['updateType'] == "Roles") {
            $update_query = $cford_db->prepare("UPDATE Roles SET roleName = ? WHERE roleId = ?;");
        ...
        if($_POST['updateType'] == "Equipment") {
            $update_query = $cford_db->prepare("UPDATE Equipment SET equipmentName = ?, equipmentTypeId = ?, quantityOnHand = ?, external = ? WHERE equipmentId = ?;");
        ...
        if($_POST['updateType'] == "EquipmentTypes") {
            $update_query = $cford_db->prepare("UPDATE EquipmentTypes SET equipmentTypeName = ? WHERE equipmentTypeId = ?;");
