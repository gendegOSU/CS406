...
if (isset($_POST['NewTaskStart'])) {
    ...
    $add_task_query = $cford_db->prepare("INSERT INTO Tasks (jobId, taskTypeId, startDate, endDate) VALUES (?, ?, ?, ?);");
...
if (isset($_POST['NewRole'])) {
    ...
    $check_task_role_query = $cford_db->prepare("SELECT * FROM TasksRoles WHERE taskId = ? and roleId = ? ");
    ...
    if (isset($check_task_role['taskId'], $check_task_role['roleId'])) {
        $update_taskRole_query = $cford_db->prepare("UPDATE TasksRoles SET roleQuantity = ((select coalesce(roleQuantity,0) from TasksRoles WHERE taskId = ? AND roleId = ?) + ?) WHERE taskId = ? AND roleId = ?;");
    ...
    $add_task_role_query = $cford_db->prepare("INSERT INTO TasksRoles (taskId, roleId, roleQuantity) VALUES (?, ?, ?);");
...
if(isset($_POST['NewRoleEmp']) && $_POST['NewRoleEmp']!=="") {
    ...
    if(in_array($_POST['NewRoleEmp'], $emp_ids)){
        ...
        $add_task_role_emp_query = $cford_db->prepare("INSERT INTO TasksRolesEmployees (taskRoleId, employeeId) VALUES ( ?, ?);");
...
if (isset($_POST['NewEquip'])) {
    ...
    $check_task_equip_query = $cford_db->prepare("SELECT * FROM TasksEquipment WHERE taskId = ? AND equipmentId = ? ;");
    ...
    if (isset($check_task_equip["taskId"], $check_task_equip["equipmentId"])) {
        $update_equip_query = $cford_db->prepare("UPDATE TasksEquipment SET quantityUsed = ((select coalesce(quantityUsed,0) from TasksEquipment WHERE taskId = ? AND equipmentId = ?) + ?)  WHERE taskId = ? AND equipmentId = ?;");
    ...
    // If it doesn't arleady exist, add it
        ...
        $add_equip_query = $cford_db->prepare("INSERT INTO TasksEquipment (taskId, equipmentId, quantityUsed) VALUES (?, ?, ?);");
...
if (isset($_POST['NewJobName'])) {
    ...
    // If all is good, add the new job
        ...
        $add_job_query = $cford_db->prepare("INSERT INTO Jobs (jobName, customerId, startDate, endDate, locationName, streetAddress, city, state, status, comments)
            VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ? );");
...
if (isset($_POST['NewTaskTypeName'])) {
    ...
    $add_task_type_query = $cford_db->prepare("INSERT INTO TaskTypes (taskTypeName) VALUES(?);");
...
if (isset($_POST['NewCustomerName']) && !isset($_POST['NewJobName'])) {
    ...
    $add_customer_query = $cford_db->prepare("INSERT INTO Customers (customerName, phoneNumber, email) VALUES(?, ?, ?);");
...
if (isset($_POST['NewFirstName'])) {
    ...
    $add_employee_query = $cford_db->prepare("INSERT INTO Employees(firstName, lastName) VALUES(?, ?);");
...
if (isset($_POST['NewEmployeeId'],$_POST["NewRoleId"])) {
    ...
    $add_employee_role_query = $cford_db->prepare("INSERT INTO EmployeesRoles(employeeId, roleId) VALUES(?, ?);");
...
if (isset($_POST['NewRoleName'])) {
    ...
    $add_role_query = $cford_db->prepare("INSERT INTO Roles(roleName) VALUES(?);");
...
if (isset($_POST['NewEquipmentName'])) {
    $add_equipment_query = $cford_db->prepare("INSERT INTO Equipment(equipmentName, equipmentTypeId, quantityOnHand, external) VALUES(?, ?, ?, ? );");
...
if (isset($_POST['NewEquipmentTypeName'])) {
    ...
    $add_equipment_type_query = $cford_db->prepare("INSERT INTO EquipmentTypes(equipmentTypeName) VALUES(?);");
